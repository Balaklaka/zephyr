...
#define LOG_MODULE_NAME bt_mesh_op_agg_cli
			 ...
			 struct net_buf_simple *buf)
{
	...
	if (!bt_mesh_msg_ack_ctx_match(&cli.ack_ctx,
				       ...
				       NULL)) {
		BT_WARN("Unexpected Opcodes Aggregator Status");
		return -ENOENT;
	}
	...
	while (buf->len > 0) {
		...
		if (err) {
			BT_ERR("Cannot decode aggregated message %d", err);
			bt_mesh_op_agg_ctx_reinit();
			return -EINVAL;
		}

		if (agg->srcs->len < 2) {
			BT_ERR("Mismatch in sources address buffer");
			bt_mesh_op_agg_ctx_reinit();
			return -ENOENT;
		}
		...
		if (err) {
			BT_ERR("Opcodes Aggregator receive error %d", err);
			bt_mesh_op_agg_ctx_reinit();
			return err;
		}
	}
...
static int op_agg_cli_init(struct bt_mesh_model *model)
{
	if (!bt_mesh_model_in_primary(model)) {
		BT_ERR("Opcodes Aggregator Client only allowed in primary element");
		return -EINVAL;
	}
				 ...
				 uint16_t elem_addr)
{
	...
	if (!BT_MESH_ADDR_IS_UNICAST(elem_addr)) {
		BT_ERR("Element address shall be a unicast address");
		return -EINVAL;
	}

	if (agg->initialized) {
		BT_ERR("Opcodes Aggregator is already configured");
		return -EALREADY;
	}
...
int bt_mesh_op_agg_cli_seq_send(void)
{
	...
	if (!agg->initialized) {
		BT_ERR("Opcodes Aggregator not initialized");
		return -EINVAL;
	}
