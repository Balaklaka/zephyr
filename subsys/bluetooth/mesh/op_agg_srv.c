...
#define LOG_MODULE_NAME bt_mesh_op_agg_srv
			   ...
			   struct net_buf_simple *buf)
{
	...
	if (!BT_MESH_ADDR_IS_UNICAST(elem)) {
		BT_WARN("Address is not unicast, ignoring.");
		return -EINVAL;
	}
	...
	while (buf->len > 0) {
		...
		if (err) {
			BT_ERR("Unable to parse Opcodes Aggregator Sequence message (err %d)", err);
			return err;
		}
	}
...
send:
	...
	if (err) {
		BT_ERR("Unable to send Opcodes Aggregator Status");
		return err;
	}
...
static int op_agg_srv_init(struct bt_mesh_model *model)
{
	if (!bt_mesh_model_in_primary(model)) {
		BT_ERR("Opcodes Aggregator Server only allowed in primary element");
		return -EINVAL;
	}
