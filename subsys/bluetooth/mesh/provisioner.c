...
static void prov_capabilities(const uint8_t *data)
{
	...
	BT_DBG("Static OOB Type:   0x%02x", caps.oob_type);
	...
	if (atomic_test_bit(bt_mesh_prov_link.flags, REPROVISION)) {
		if (!bt_mesh_prov_link.addr) {
			...
			if (!bt_mesh_prov_link.addr) {
				BT_ERR("Failed allocating address for node");
		...
		if (prov_device.node == NULL) {
			BT_ERR("Failed allocating node 0x%04x", bt_mesh_prov_link.addr);
...
static void send_prov_data(void)
{
	...
	BT_DBG("DevKey: %s", bt_hex(prov_device.new_dev_key, 16));
				...
				bool composition_change)
{
	...
	if (!prov_device.node) {
		BT_ERR("No CDB node for 0x%04x", srv->addr);
		return -ENOENT;
	}
