...
static void seg_tx_send_unacked(struct seg_tx *tx)
{
	...
	if (BT_MESH_ADDR_IS_UNICAST(tx->dst) &&
		...
		BT_ERR("Ran out of retransmit without progress attempts");
...
static void seg_discard(struct k_work *work)
{
	...
	BT_WARN("SAR Discard timeout expired");
...
static void seg_ack(struct k_work *work)
{
	...
	if (rx->attempts_left == 0) {
		BT_DBG("Ran out of retransmit attempts");
		return;
	}
...
found_rx:
	...
	BT_DBG("discard timeout %u", BT_MESH_SAR_RX_DISCARD_TIMEOUT_MS);
