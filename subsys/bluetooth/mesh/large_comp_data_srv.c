...
#define LOG_MODULE_NAME bt_mesh_large_comp_data_srv
				      ...
				      struct net_buf_simple *buf)
{
	...
	BT_DBG("page %u offset %u", page, offset);
	...
	if (offset < total_size) {
		...
		if (err && err != -E2BIG) {
			BT_ERR("comp_get_page_0 returned error");
			return err;
		}
	}

	if (bt_mesh_model_send(model, ctx, &rsp, NULL, NULL)) {
		BT_ERR("Unable to send Large Composition Data Status");
	}
				      ...
				      struct net_buf_simple *buf)
{
	...
	if (page >= 128U && atomic_test_bit(bt_mesh.flags, BT_MESH_METADATA_DIRTY)) {
		BT_DBG("Models Metadata Page 128");
		page = 128U;
	} else if (page != 0U) {
		BT_DBG("Models Metadata Page %u not available", page);
		page = 0U;
	}
	...
	if (atomic_test_bit(bt_mesh.flags, BT_MESH_METADATA_DIRTY) == (page == 0U)) {
		...
		if (err) {
			BT_ERR("Unable to get stored models metadata");
			return err;
		}
		...
		if (offset < total_size) {
			...
			if (err && err != -E2BIG) {
				BT_ERR("Failed to get Models Metadata Page 0: %d", err);
				return err;
			}
		}
	}
	...
	if (bt_mesh_model_send(model, ctx, &rsp, NULL, NULL)) {
		BT_ERR("Unable to send Models Metadata Status");
	}
...
static int large_comp_data_srv_init(struct bt_mesh_model *model)
{
	if (!bt_mesh_model_in_primary(model)) {
		BT_ERR("Large Composition Data Server only allowed in primary element");
		return -EINVAL;
	}
