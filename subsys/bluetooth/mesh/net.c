		      ...
		      enum bt_mesh_net_if net_if)
{
	...
	if (err == -EAGAIN) {
		...
		*/
		...
		BT_DBG("Not relaying message rejected by the Transport layer");
		return;
	}
...
{	int err;
	...
	if (!err) {
		BT_DBG("DevKey candidate recovered from storage");
...
void bt_mesh_net_dev_key_cand_store(void)
{
	...
	if (err) {
		BT_ERR("Failed to update DevKey candidate value");
	} else {
		BT_DBG("Stored DevKey candidate value");
	}
}
#endif
