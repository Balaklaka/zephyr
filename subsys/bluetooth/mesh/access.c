...
static inline size_t bt_mesh_comp_elem_size(const struct bt_mesh_elem *elem)
{
			      ...
			      void *user_data)
{
	...
	if ((net_buf_simple_tailroom(buf) - 4) < model_size) {
		BT_DBG("Model metadata didn't fit in the buffer");
		return -E2BIG;
	}
...
int bt_mesh_metadata_get_page_0(struct net_buf_simple *buf, size_t offset)
{
	...
	for (i = 0; i < comp->elem_count; i++) {
		...
		mod_count_ptr = net_buf_simple_add_u8_offset(buf, 0, &offset);
		vnd_count_ptr = net_buf_simple_add_u8_offset(buf, 0, &offset);
			 ...
			 size_t *offset)
{
	...
	if (net_buf_simple_tailroom(buf) < (elem_size + BT_MESH_MIC_SHORT)) {
		if (IS_ENABLED(CONFIG_BT_MESH_LARGE_COMP_DATA_SRV)) {
			 ...
			 */
			BT_DBG("Element 0x%04x didn't fit in the Data field",
			       elem->addr);
			return 0;
		}
...
int bt_mesh_comp_data_get_page_0(struct net_buf_simple *buf, size_t offset)
{
	...
	if (offset <= 10) {
		...
		net_buf_simple_add_le16_offset(buf, CONFIG_BT_MESH_CRPL,
					       &offset);
			      ...
			      struct bt_mesh_elem *elem, uint32_t opcode)
{
	...
	if (!op) {
		BT_ERR("No OpCode 0x%08x for elem 0x%02x", opcode, elem->addr);
	...
	if (!model_has_dst(model, addr)) {
		BT_ERR("Invalid address 0x%02x", addr);
	...
	if ((op->len >= 0) && (buf->len < (size_t)op->len)) {
		...
		BT_ERR("Invalid message size for OpCode 0x%08x", opcode);
		return ACCESS_STATUS_MESSAGE_NOT_UNDERSTOOD;
	}
...
int bt_mesh_model_recv(struct bt_mesh_msg_ctx *ctx, struct net_buf_simple *buf)
{
	...
	if (BT_MESH_ADDR_IS_UNICAST(ctx->recv_dst)) {
		...
		if (index >= dev_comp->elem_count) {
			BT_ERR("Invalid address 0x%02x", ctx->recv_dst);
		...
		for (index = 0; index < dev_comp->elem_count; index++) {
			...
			if (bt_mesh_elem_find_group(elem, ctx->recv_dst)) {
...
int bt_mesh_comp_store(void)
{
	...
	if (err) {
		BT_ERR("Failed to store composition data: %d", err);
	} else {
		BT_DBG("Stored composition data");
	}
...
int bt_mesh_models_metadata_store(void)
{
	...
	BT_DBG("bt/mesh/metadata total %d", total_size);
	...
	if (err == -E2BIG) {
		BT_ERR("Metadata too large");
		return err;
	}
	if (err) {
		BT_ERR("Failed to read models metadata: %d", err);
		return err;
	}

	BT_DBG("bt/mesh/metadata len %d", buf.len);

	err = settings_save_one("bt/mesh/metadata", buf.data, buf.len);
	if (err) {
		BT_ERR("Failed to store models metadata: %d", err);
	} else {
		BT_DBG("Stored models metadata");
	}
...
int bt_mesh_models_metadata_read(struct net_buf_simple *buf, size_t offset)
{
	...
	if (err) {
		BT_ERR("Failed reading models metadata: %d", err);
		return err;
	}
	...
	BT_DBG("metadata read %d", buf->len);
...
void bt_mesh_models_metadata_clear(void)
{
	...
	if (err) {
		BT_ERR("Failed to clear models metadata: %d", err);
	} else {
		BT_DBG("Cleared models metadata");
	}
