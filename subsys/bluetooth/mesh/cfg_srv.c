			     ...
			     struct net_buf_simple *buf)
{
	...
	BT_DBG("Preparing Composition data page %d", page);
	...
	if (IS_ENABLED(CONFIG_BT_MESH_COMP_PAGE_1) && page == 1) {
		...
		if (err < 0) {
			BT_ERR("Unable to get composition page 1");
			return err;
		}
		...
		if (err) {
			BT_ERR("Unable to get stored composition data");
			return err;
		}
...
static uint8_t mod_bind(struct bt_mesh_model *model, uint16_t key_idx)
{
	...
	for (i = 0; i < ARRAY_SIZE(model->keys); i++) {
		BT_DBG("model %p id 0x%04x i %d key 0x%03x", model, model->id, i, model->keys[i]);
			...
			struct net_buf_simple *buf)
{
	...
	if (mod->flags & BT_MESH_MOD_DEVKEY_ONLY) {
		BT_ERR("Client tried to bind AppKey to DevKey based model");
