...
#define LOG_MODULE_NAME bt_mesh_rpr_srv
			     ...
			     enum bt_mesh_rpr_status status)
{
	...
	net_buf_simple_add_u8(&rsp, time);
...
static void link_report_send(void)
{
	...
	BT_DBG("%u %u", srv.link.status, srv.link.state);
...
static void scan_report_send(void)
{
	...
	for (i = 0; i < srv.scan.max_devs; ++i) {
		...
		if (err) {
			atomic_clear_bit(srv.flags, SCAN_REPORT_PENDING);
			BT_DBG("tx failed: %d", err);
			break;
		}

		BT_DBG("Reported unprov #%u", i);
		dev->flags |= BT_MESH_RPR_UNPROV_REPORTED;
		break;
	}
}
		       ...
		       enum prov_bearer_link_status reason)
{
	...
	BT_DBG("status: %u reason: %u", status, reason);
			     ...
			     struct net_buf_simple *buf)
{
	...
	BT_DBG("max %u devs, %u s %s", max_devs, timeout,
	       uuid ? bt_hex(uuid, 16) : "");
				      ...
				      struct net_buf_simple *buf)
{
		...
		if (prov->uri && get_ad_type(ad, ad_count, BT_DATA_URI)) {
			...
			if (uri_len < CONFIG_BT_MESH_RPR_SRV_AD_DATA_MAX - 2) {
				...
				net_buf_simple_add_mem(&rsp, prov->uri,
						       uri_len);
				BT_DBG("URI added: %s", prov->uri);
			} else {
				BT_WARN("URI data won't fit in scan report");
			}
		}
	...
	if (BT_DBG_ENABLED) {
		...
		BT_DBG("%s AD types: %s", bt_uuid_str(&uuid_repr.uuid),
		       bt_hex(ad, ad_count));
	}

	if (timeout < BT_MESH_RPR_EXT_SCAN_TIME_MIN ||
	    timeout > BT_MESH_RPR_EXT_SCAN_TIME_MAX) {
		BT_ERR("Invalid extended scan timeout %u", timeout);
		return -EINVAL;
	}
			    ...
			    struct net_buf_simple *buf)
{
	...
	if (srv.link.state == BT_MESH_RPR_LINK_CLOSING ||
	    srv.link.state == BT_MESH_RPR_LINK_SENDING) {
		status = BT_MESH_RPR_ERR_INVALID_STATE;
		BT_ERR("Invalid state: %u", srv.link.state);
		goto rsp;
	}
	...
	if (is_refresh_procedure) {
		...
		if (refresh > BT_MESH_RPR_NODE_REFRESH_COMPOSITION) {
			BT_ERR("Invalid refresh: %u", refresh);
			return -EINVAL;
		}

		if (refresh == BT_MESH_RPR_NODE_REFRESH_COMPOSITION &&
		    !atomic_test_bit(bt_mesh.flags, BT_MESH_COMP_DIRTY)) {
			BT_WARN("Composition data page 128 missing");
			status = BT_MESH_RPR_ERR_LINK_CANNOT_OPEN;
			goto rsp;
		}

		BT_DBG("Node Refresh: %u", refresh);
	...
	if (buf->len) {
		...
		if (!timeout || timeout > 0x3c) {
			BT_ERR("Invalid timeout: %u", timeout);
			return -EINVAL;
		}
	}

	BT_DBG("0x%04x: %s", cli.addr, bt_hex(uuid, 16));
			   ...
			   struct net_buf_simple *buf)
{
	...
	if (srv.link.state != BT_MESH_RPR_LINK_ACTIVE) {
		BT_WARN("Sending PDU while busy (state %u)", srv.link.state);
		return 0;
	}
	...
	if (pdu_num != srv.link.tx_pdu + 1) {
		BT_WARN("Invalid pdu number: %u, expected %u", pdu_num,
			srv.link.tx_pdu + 1);
		outbound_pdu_report_send();
		return 0;
	}
		  ...
		  struct bt_data *ad)
{
	...
	BT_DBG("Unprov #%u: %s OOB: 0x%04x %s", dev - &srv.scan.devs[0],
				...
				struct net_buf_simple *buf)
{
	...
	if (IS_ENABLED(CONFIG_BT_MESH_DEBUG_MODEL)) {
		...
		BT_DBG("Is %s", bt_uuid_str(&uuid_repr.uuid));
	}
	...
	while (srv.scan.ad_count && pull_ad_data(buf, &ad)) {
		...
		if (ad.data_len + 2 >
			...
			BT_WARN("Can't fit AD 0x%02x in scan report", ad.type);
			continue;
		}
...
static int rpr_srv_init(struct bt_mesh_model *mod)
{
	if (mod->elem_idx || srv.mod) {
		BT_ERR("Remote provisioning server must be initialized "
		       "on first element");
		return -EINVAL;
	}
