...
{
	if (buf->len < 6) {
		BT_ERR("No more elements to pull or missing data");
		return NULL;
	}
	size_t elem_size = 0;
	...
	for (i = 0; i < elem->nsig + elem->nvnd; i++) {
		...
		BT_DBG("header %d, cor_present %d, fmt %d, ext_item_cnt %d",
	...
	struct bt_mesh_comp_p1_elem *elem, struct bt_mesh_comp_p1_model_item *item)
{
	...
	BT_DBG("N_SIG %d, N_VND %d, buf len=%d:0x%s",
	...
	struct bt_mesh_comp_p1_model_item *item, struct bt_mesh_comp_p1_item_short *ext_item)
{
	...
	BT_DBG("Correspondence ID %s, format %s, extended items count=%d",
...
{
	if (item->_buf->len < 2) {
		BT_ERR("Missing data, buf len=%d", item->_buf->len);
		return NULL;
	}
	...
	struct bt_mesh_comp_p1_model_item *item, struct bt_mesh_comp_p1_ext_item *ext_item)
{
	if (item->_buf->len < 1) {
		...
		BT_DBG("Item in short format");
		ext_item->type = SHORT;
		comp_p1_pull_item_short(item, &ext_item->short_item);
	} else {
		BT_DBG("Item in long format");
		ext_item->type = LONG;
