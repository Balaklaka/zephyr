...
/**
 * @file
 * @defgroup bt_mesh_dfu_cli Device Firmware Upgrade Client model
 * @ingroup bt_mesh_dfu
 * @{
 * @brief API for the Bluetooth Mesh Firmware Upgrade Client model
 */
 ...
 *  @param total   Total number of images on the target.
...
struct bt_mesh_dfu_cli_cb {
	...
	/** @brief DFU ended.
	 *
	 *  Called when the DFU transfer ends, either because all targets were
	...
	/** @brief DFU transfer applied on all active targets.
	 *
	...
	/** @brief DFU transfer confirmed on all active targets.
	 *
	 *  Called at the end of the apply procedure started by @ref
	...
	/** @brief DFU target was lost.
	 *
	 *  A DFU target was dropped from the receiver list. The target's
	 *  @c status is set to reflect the reason for the failure.
	 *
	 *  @param cli    DFU Client model instance.
	 *  @param target DFU target that was lost.
	 */
	void (*lost_target)(struct bt_mesh_dfu_cli *cli,
			    struct bt_mesh_dfu_target *target);
};

/** DFU Client model instance.
 *
 ...
 *
 *  A transfer can only be applied after it has ended successfully. The DFU
 *  Client's @c applied callback is called at the end of the apply procedure.
 *
 *  @param cli DFU Client model instance.
 *
 *  @return 0 on success, or (negative) error code otherwise.
...
/** @brief Confirm that the active transfer has been applied on the targets.
 *
 *  A transfer can only be confirmed after it has been applied. The DFU
 *  Client's @c confirmed callback is called at the end of the confirm
 *  procedure.
 *
 *  Targets that have reported the effect as @ref BT_MESH_DFU_EFFECT_UNPROV
 ...
 *  @param cli DFU Client model instance.
 *
 *  @return true if the BLOB client is currently participating in a transfer,
 *          false otherwise.
 */
 ...
 *
 *  The metadata check procedure allows the DFU client to check whether a target
 ...
 *  @param cli     DFU Client model instance.
 *  @param ctx     Message context.
 *  @param img_idx Target's image index to check.
