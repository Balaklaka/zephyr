...
/**
 * @file
 * @defgroup bt_mesh_dfu_srv Device Firmware Upgrade Server model
 * @ingroup bt_mesh_dfu
 * @{
 * @brief API for the Bluetooth Mesh Device Firmware Upgrade Server model
 */
...
/** @brief DFU Server event callbacks. */
struct bt_mesh_dfu_srv_cb {
	 ...
	 *
	 *  Called when the DFU Server is ready to start a new DFU transfer.
	 ...
	 *    image, and there's no need to transfer it again. The dfu model
	 ...
	 *
	 *  If the transfer fails, the DFU Server will be available for new
	 ...
	 *
	 *  If the device reboots in the middle of a transfer, the DFU Server
	 *  calls this function when the Mesh subsystem is started.
	 *
...
/** @brief Check whether the DFU Server is busy processing a transfer.
 *
 *  @param srv DFU Server instance.
 *
