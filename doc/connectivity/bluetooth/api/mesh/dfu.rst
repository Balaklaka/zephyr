...
Firmware metadata
   ...
   The Bluetooth mesh DFU subsystem in Zephyr :c:struct:`provides <bt_mesh_dfu_metadata>` its own metadata format together with a set of related functions that can be used by an end product. The support for it can be enabled using :option:`CONFIG_BT_MESH_DFU_METADATA`. The format of the metadata is the following:
